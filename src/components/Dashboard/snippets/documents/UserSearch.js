...
const UserSearch = ({ selectUser }) => {
  ...
  const renderSearchingState = () => {
    ...
    return users.searchResults.data.length;
  };

  return (
    <>
      <div
        className={`input-container form-input no-padding smaller-placeholder above-all ${
      ...
      >
        ...
        {((users?.searchResults && searchTerm !== "") || users?.searching) && (
            ...
            {users.searching ? (
              ...
              <>
                <div className="display-flex align-items-center">
                  <p className="size-pointeight-rem bold right-margin-10">
                    USER SUGGESTIONS
                  </p>
                ...
                {users.searchResults?.data.map((user, index) => (
                  <UserSearchResult
                    ...
                    length={users.searchResults.data.length}
                  />
                ))}
              </>
            )}
