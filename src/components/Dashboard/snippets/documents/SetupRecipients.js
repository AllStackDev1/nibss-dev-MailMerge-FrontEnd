...
const SetupRecipients = ({ document, addRecipient, addAllRecipient }) => {
  ...
  const fetchMoreRecipients = (e) => {
    if (
      ...
      if (
        recipients.fetching === false &&
        recipients.documentRecipients.pagination.current !==
          recipients.documentRecipients.pagination.number_of_pages
      ) {
        dispatch(
          recipientActions.fetchPage(
            recipients.documentRecipients.pagination.next,
            "document"
          )
        );
      }
  ...
  const renderRecipients = () => {
    ...
    return (
      <>
        <div className="cursor-pointer bottom-margin-20">
          <input
            type="checkbox"
            id="recipient-all"
            className="checkbox-s"
            indeterminate={document.recipients.length !== toLoop.data.length}
            onChange={() => addAllRecipient(toLoop.data)}
            checked={document.recipients.length === toLoop.data.length}
            data-test="checkbox-input"
          />
          <label
            htmlFor="recipient-all"
            className="no-shrink absolute left center-item-vertically"
          ></label>
            <div
              onClick={() => addAllRecipient(toLoop.data)}
              className="left-padding-30 display-flex full-width height-40 align-items-center right-padding-50 border-box above"
            >
              <div className="no-shrink left-margin-20 width-50-percent size-pointeight-rem bold capitalize">
                Select All Recipient
              </div>
            </div>
        </div>
        {toLoop.data.map((recipient, index) => (
          <div key={index} className="cursor-pointer bottom-margin-20">
            <input
              type="checkbox"
              id={`recipient-${index}`}
              className="checkbox-s"
              onChange={() => addRecipient(recipient)}
              checked={
                document.recipients.findIndex(
                  (rec) => rec._id === recipient._id
                ) !== -1
              }
              data-test="checkbox-input"
            />
            ...
            <div
              ...
              className="left-padding-30 display-flex full-width align-items-center right-padding-50 border-box above"
            >
              <Profile
              ...
              <div className="no-shrink width-30-percent size-pointeight-rem">
                <p className="size-pointeight-rem light-gray-color truncate">
                  {recipient.email}
                </p>
              </div>
            </div>
  ...
  return (
    <>
      <p className="gray-color size-onepointtwo-rem bold text-center top-margin-30">
        Select Recipients
        <span className="light size-pointnine-rem">
          {document?.recipients.length > 0
            ? ` - ${document?.recipients.length} selected`
            : ""}
        </span>
      </p>
      <p className="light-gray-color size-pointeight-rem text-center top-margin-5">
        Tell us who will be getting this documenmts
      </p>
      ...
      <div className="display-flex width-85-percent space-between top-margin-30">
        ...
        >
          <img
            src={require(`images/icons/dashboard/filter.svg`)}
            className={`height-15 right-margin-10`}
            alt="Filter"
          />
          Filter By
                      ...
                      <input
                        ...
                        onChange={() =>
                          filter.includes(tag.name)
                            ? setFilter((f) =>
                                f.filter((item) => item !== tag.name)
                              )
                            : setFilter((f) => [...f, tag.name])
                        }
                        checked={filter.includes(tag.name)}
                        className="checkbox-s"
                      />
                      <label
                        htmlFor={`tag-${index}`}
      ...
      <div
        className="full-height width-85-percent top-margin-40 border-box overflow-auto-y custom-scrollbar"
        onScroll={fetchMoreRecipients}
      >
        {renderRecipients()}
      </div>
    </>
