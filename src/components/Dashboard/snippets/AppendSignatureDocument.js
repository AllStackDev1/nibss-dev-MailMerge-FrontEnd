  ...
  function onDocumentLoadSuccess({ numPages: numPs }) {
    ...
    setTimeout((e) => {
      document.document.signatories.forEach((signatory, i) => {
        ...
        if (
          refsFull.current[signatory.page ? parseInt(signatory.page) : 0]
            ?.current
        ) {
          documentCopy.document.signatories[i] = {
            ...signatory,
            absolute_x_coordinate:
              (signatory.x_coordinate /
                refsFull.current[signatory.page ? parseInt(signatory.page) : 0]
                  .current.offsetWidth) *
              refs.current[signatory.page ? parseInt(signatory.page) : 0]
                .current.offsetWidth,
            absolute_y_coordinate:
              (signatory.y_coordinate /
                refsFull.current[signatory.page ? parseInt(signatory.page) : 0]
    ...
    >
      {[...Array(numPages)].map((el, index) => (
        <PageContainer
          ...
          className={`${index} ${
            isNumPagesSet ? width75percent : "full-width"
          } bottom-margin-20`}
        >
          <Page
            width={pageWidth}
