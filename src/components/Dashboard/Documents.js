...
const Documents = withRouter(({ location }) => {
  ...
  const addRecipient = (recipient) => {
    setDocument((d) => {
      d.recipients.findIndex((rec) => rec._id === recipient._id) !== -1
        ? d.recipients.splice(
            d.recipients.findIndex((rec) => rec._id === recipient._id),
            1
          )
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    r.onloadend = function (e) {
      setDocumentFiles((d) => [...d, e.target.result]);
    };
  ...
  const viewPage = (p) => {
    if (
      p <= documents.documents.pagination.number_of_pages &&
      p !== documents.documents.pagination.current
    ) {
  ...
  return (
    <div
      ref={page}
      className="full-width full-height custom-scrollbar overflow-auto-y"
    >
      {uploadingDocument ? (
        <>
          {step < 5 && (
          ...
          <Container
            className={`${(step === 3 || step === 4) && documentContainerClass} 
                            ...
                            } onboarding width-80-percent bottom-padding-80 display-flex flex-direction-column 
          ...
          >
            ...
            {step === 2 && (
              <SetupRecipients
                ...
                addAllRecipient={addAllRecipient}
              />
            )}
            {step === 3 && (
              <SigningSetup
            ...
            {step === 4 && (
              <div className="full-width width-85-percent margin-auto">
                ...
                <Gray className="min-height-350 display-flex top-margin-40 border-box bottom-padding-30 padding-10 border-radius-15">
                  <RTE
                    onChange={(documentBody) =>
                      setDocument({ ...document, documentBody })
                    }
                  />
                </Gray>
            ...
            {step < 5 && (
              ...
              >
                ...
                <button
                  onClick={() => {
                    step < 4 ? setStep((s) => s + 1) : prepareDocument();
                  }}
        ...
        <>
          ...
          <div className="full-width border-box left-padding-30 right-padding-30">
            ...
            >
              <img
                src={require(`images/icons/dashboard/upload.svg`)}
                className="height-20 right-margin-10"
                alt="NIBSS Upload Document"
              />
              Upload a document
            </UploadButton>
